...
//    producer = new Producer<String, byte[]>(producer_config);
        LOG.info("Call KafkaProucer with " + producer_config.toString());
        producer = new KafkaProducer<String, byte[]>(producer_config);
...
//              JSON.serializeToBytes(dp));
            ...
            final ProducerRecord<String, byte[]> data = new ProducerRecord<String, byte[]>(topic,
                    Integer.toString(Math.abs(hash)),
                    JSON.serializeToBytes(dp));
            LOG.info("Print Message by lqing: "+data.toString());
            producer.send(data);
