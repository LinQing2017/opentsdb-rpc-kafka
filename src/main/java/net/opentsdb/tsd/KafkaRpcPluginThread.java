...
//    private TopicFilter topic_filter;
...
//  private ConsumerConnector consumer;
    ...
    public KafkaRpcPluginThread(final KafkaRpcPluginGroup group,
        ...
        LOG.info("Strat init KafkaRpcPluginThread. ThreadID " + threadID);
...
//        LOG.info("Debug Message by lqing: Init Topic_Filter, with " + topics);
//        topic_filter = new Whitelist(topics);
//        LOG.info("Debug Message by lqing: Finish Topic_Filter." + topic_filter.regex());
        this.topics=topics;
        ...
        LOG.info("Debug Message by lqing: Finish init KafkaRpcPluginThread");
    }
...
//  }
    ...
    public void run() {
        ...
        KafkaConsumer<byte[], byte[]> stream = null;
        try {

            final Properties consumerProperties = buildConsumerProperties();
            LOG.info("Debug message print by lqing : Create KafkaConsumer, with Config : " + consumerProperties.toString());
            stream = new KafkaConsumer<byte[], byte[]>(consumerProperties);
            LOG.info("Debug message print by lqing : subscrible Topic : " + topics);
            stream.subscribe(Arrays.asList(topics.trim()));
            int errorCount = 0;
            long nanoCtr = System.nanoTime();
            while (true) {
                ConsumerRecords<byte[], byte[]> consumerRecords = stream.poll(3000);
                        ...
                        switch (consumer_type) {
                            ...
                            case ROLLUP:
                                ...
                                final List<TypedIncomingData> eventList =
                                        deserializer.deserialize(this, message.value());
                                if (eventList == null) {
